...
import { readableStreamToString, CircleCiError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new CircleCiError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new CircleCiError(e, e.response?.data)
                }
                if (innerError instanceof CircleCiError) {
                    // Got "CircleCiError" from the above try block
                    throw innerError;
                }
